...
func TestStoppingRuleHoldsAcrossRepeatedDeliveries(t *testing.T) {
	...
	for i, eventID := range []string{"evt_repeat_2", "evt_repeat_3"} {
		...
		// attempt_count is the number of times this payment has been SEEN, so it
		// keeps climbing past the budget rather than pinning at it. That is what
		// makes repeated deliveries of a stopped payment visible in the log.
